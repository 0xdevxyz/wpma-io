...
/**
 * Pull health data from a single WordPress site via the WPMA REST API.
 * Returns the raw health payload on success, or an error status string.
 */
async function pullSiteHealth(site) {
    const siteUrl = (site.site_url || '').replace(/\/$/, '');
    const apiKey  = site.api_key || '';

    // 1. Try authenticated /health endpoint
    try {
        ...
        if (res.status === 403) {
            // Key mismatch — fall through to status check
        }
    } catch (_) {
        // Network / timeout error — fall through
    }

    // 2. Fallback: unauthenticated /status endpoint to detect plugin presence
    try {
...
/**
 * Persist health data returned by the /health endpoint into the sites table.
 */
...
// POST /api/v1/sync/sites/:siteId/sync
...
// GET /api/v1/sync/sites/:siteId/synced-data
router.get('/sites/:siteId/synced-data', async (req, res) => {
    try {
        const userId  = getUserId(req);
        const { siteId } = req.params;
