...
// ─── Client-Auth-Middleware (für Client-Portal-Endpunkte) ─────────────────────
...
// ─── CLIENT AUTH: Eigene Sites & Daten ───────────────────────────────────────
...
router.get('/sites', authenticateClient, async (req, res) => {
    try {
        const result = await query(
...
// ─── AGENCY AUTH: Client-Verwaltung ──────────────────────────────────────────

router.use(authenticateToken);

// Alle Clients einer Agentur
...
// Sites einem Client zuweisen
router.get('/:clientId/sites', async (req, res) => {
    try {
        const userId = getUserId(req);
        const { clientId } = req.params;
...
router.post('/:clientId/sites', async (req, res) => {
    try {
        const userId = getUserId(req);
        const { clientId } = req.params;
        ...
        // Replace access (delete all, re-insert)
